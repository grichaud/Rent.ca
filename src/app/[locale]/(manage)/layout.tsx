...
import { ManageSidebar } from '@/features/landlord-manage/components/manage-sidebar'
...
export default function ManageLayout({ children }: { children: React.ReactNode }) {
  ...
  const { user, isLoading, isAuthenticated, isLandlord } = useAuth()
  const router = useRouter()
  const locale = useLocale()

  useEffect(() => { setMounted(true) }, [])

  // Role guard: redirect non-landlords to renter portal
  useEffect(() => {
    if (isLoading) return
    if (!isAuthenticated) {
      router.replace(`/${locale}/login`)
    } else if (!isLandlord) {
      router.replace(`/${locale}/renter`)
    }
  }, [isLoading, isAuthenticated, isLandlord, router, locale])
  ...
  const sidebarTrigger = mounted ? (
    <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
      ...
      >
        ...
        <ManageSidebar onNavClick={() => setSheetOpen(false)} />
      </SheetContent>
    </Sheet>
  ) : (
    <button
