...
import { getListingForEdit, updateListing } from '@/features/landlord-manage/services/landlord-service'
import { uploadPhoto, deleteStorageFile, savePropertyPhotos, loadPropertyPhotos } from '@/features/landlord-manage/services/photo-service'
...
export default function EditListingPage() {
  ...
  const loadListing = useCallback(async () => {
    setIsLoading(true)
    setLoadError(null)
    const data = await getListingForEdit(propertyId)
    if (!data) {
      setLoadError('Listing not found or you do not have permission to edit it.')
      setIsLoading(false)
      return
    }
    setBasicInfo(data.basicInfo)
    setUnits(data.units.length > 0 ? data.units : [])
    setAmenities(data.amenityIds)
    setOriginalStatus(data.status)

    // Load existing photos from DB
    const existingPhotos = await loadPropertyPhotos(propertyId)
    setPhotos(existingPhotos.map((p) => ({
      id: p.id,
      previewUrl: p.url,
      fileName: p.url.split('/').pop() ?? 'photo',
  ...
  async function handleSave(status: string) {
    ...
    const { error } = await updateListing({
      ...
      status: status as 'active' | 'draft' | 'inactive' | 'archived',
    })
    if (error) {
    ...
    // 2. Upload new photos, keep existing ones
    const finalPhotos = []
    for (const photo of photos) {
      if (photo.file) {
        // New photo — upload to storage
        const res = await uploadPhoto(photo.file, user.id, propertyId)
        if (res.error) {
          setIsSubmitting(false)
    ...
    // 3. Delete removed photos from storage (load original list, diff)
    const originalPhotos = await loadPropertyPhotos(propertyId)
    const keptUrls = new Set(finalPhotos.map((p) => p.storageUrl))
    for (const orig of originalPhotos) {
      if (!keptUrls.has(orig.url)) {
        await deleteStorageFile(orig.url)
      }
    }

    // 4. Save all photo records to DB
    const saveRes = await savePropertyPhotos(propertyId, finalPhotos)
    if (saveRes.error) {
      setIsSubmitting(false)
  ...
  async function handleSaveDraft() {
    await handleSave(originalStatus)
  }
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-20">
        ...
        <span className={cn('ml-3 text-sm', glassColors.text.muted)}>Loading listing...</span>
      </div>
    )
  }
  ...
  if (loadError) {
    return (
      <div className="max-w-2xl mx-auto py-20 text-center">
        ...
        <button
          type="button"
          onClick={() => router.push(`/${locale}/landlord/listings`)}
          className={cn(glass.button, 'px-5 py-2.5 text-sm font-medium text-gray-600 dark:text-white/70')}
        >
          Back to Listings
        </button>
      </div>
