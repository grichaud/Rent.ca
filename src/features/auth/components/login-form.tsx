...
export function LoginForm() {
  ...
  // Extract locale from path like '/en/login' → 'en'
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    router.push(returnTo ?? ROUTES.dashboard)
  }

  const handleGoogleSignIn = async () => {
    setIsLoading(true)
    const result = await signInWithGoogle('renter', locale)
    if (result.error) {
      setErrors({ general: result.error })
      setIsLoading(false)
    }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate>
      ...
      <p className={cn('text-sm mb-8', glassColors.text.secondary)}>
        Sign in to your Rent.ca account
      </p>
      ...
      <div className="flex flex-col gap-4">
        ...
        <div>
          <div className="flex items-center justify-between mb-1.5">
            <label
              htmlFor="password"
              className={cn('block text-sm font-medium', glassColors.text.secondary)}
            >
              Password
            </label>
            <Link
      ...
      <div className="relative my-5">
        ...
        <div className="relative flex justify-center text-xs">
          <span className="bg-transparent px-3 text-gray-400 dark:text-white/40">or continue with</span>
        </div>
      </div>
